...
public class TranslatorVisitor implements OWLClassExpressionVisitorEx<OWLClassExpression>{
	...
	public TranslatorVisitor(OWLClassExpression inputExp, OWLReasoner reasoner, ArrayList<OWLNamedIndividual> namedElements){
	...
	public OWLClassExpression visit(OWLObjectHasSelf ce) {		
		...
		if(ce.getProperty() instanceof OWLObjectEpistemicRole){
			OWLObjectPropertyExpression prop = ((OWLObjectEpistemicRole)ce.getProperty()).getEpistemicRole();
			//			 check if \Sigma\not|=\top\squbseteq 3S.Self
			OWLClassExpression tmp=this.factory.getOWLObjectHasSelf(prop);
			if(!(this.reasoner.isEntailed(this.factory.getOWLSubClassOfAxiom(this.factory.getOWLThing(), tmp)))){
				Set<OWLIndividual> set = new HashSet<OWLIndividual>();
				OWLAxiom axiom;
				for(OWLNamedIndividual a : this.namedElements){
					axiom = this.factory.getOWLObjectPropertyAssertionAxiom(prop, a, a);
	...
	@Override
	/*
	 * This method recursivley translate the operands of a class expression (representing the intersection of these)
	 * operands and translate them into a non-epismitec class expression. Finally constructs a new class expression 
	 * from the translated operands into a non-epistmeic intersection of these translated operand i.e., it returns 
	 * the non-epistemic version of the input class expression ce
	 */
	...
	/*
	 * This method translates a class expression of form Not-C into non-epistemic equivalent by first translating C into a 
	 * non-epistemic class
	 */
	...
	//	************************************** Existential Quantification \exist R. D**********************************************************
	...
	public OWLClassExpression visit(OWLObjectSomeValuesFrom ce) {
		...
		if(ce.getProperty() instanceof OWLObjectEpistemicRole){
			OWLObjectPropertyExpression prop = ((OWLObjectEpistemicRole)ce.getProperty()).getEpistemicRole();
			//			checking if the role is equivalent to topProperty i.e., the universal role, we just remove K 
			OWLPropertyAxiom tmp = this.factory.getOWLEquivalentObjectPropertiesAxiom(ce.getProperty(),this.factory.getOWLTopObjectProperty());
			...
			//			Collection the four disjunct (see Definition of the translation function)
			Set<OWLClassExpression> disjuncts = new HashSet<OWLClassExpression>();
			//			First Disjunct d1: it is disjunctions of exp of the form {a}\sqcap\exists P.({b|KB|= P(a,b)}\sqcap Trans(D))
			...
			for(OWLNamedIndividual a:this.namedElements){
				for(OWLNamedIndividual b:this.namedElements){
					if(this.reasoner.isEntailed(this.factory.getOWLObjectPropertyAssertionAxiom(prop, a, b))){
						oneOf.add(b);
					}
				}
				if(!oneOf.isEmpty()){
					tmp1 = this.factory.getOWLObjectOneOf(a); //{b|KB|= P(a,b)}
					tmp2 = this.factory.getOWLObjectIntersectionOf(this.factory.getOWLObjectOneOf(oneOf),newFiller);//IntersectionOf({b|KB|= P(a,b)},Trans(D))
					tmp3 = this.factory.getOWLObjectSomeValuesFrom(prop, tmp2);//exist P (IntersectionOf({b|KB|= P(a,b)},Trans(D)))
					disjuncts.add(this.factory.getOWLObjectIntersectionOf(tmp1,tmp3));
				}

			}

			//			Second disjunct: it is exp of the form \exists P.({b|KB|=top\sqsubseteq\exists P.{b}}\sqcap Trans(D))
			Set<OWLIndividual> tmpOneOf1 = new HashSet<OWLIndividual>() ;
			OWLClassExpression tmpExp;
			for(OWLNamedIndividual b:this.namedElements){
				tmpExp = this.factory.getOWLObjectSomeValuesFrom(prop, this.factory.getOWLObjectOneOf(b));
				if(this.reasoner.isEntailed(
						this.factory.getOWLSubClassOfAxiom(
								this.factory.getOWLThing(),tmpExp))){
					tmpOneOf1.add(b);
				}
				if(!tmpOneOf1.isEmpty()){
					disjuncts.add(this.factory.getOWLObjectSomeValuesFrom(prop, 
							this.factory.getOWLObjectIntersectionOf(this.factory.getOWLObjectOneOf(tmpOneOf1),newFiller)));

				}
			}
			//			Third disjunct: it is expression of the form {a|KB|=\top\sqsubseteq\exist P^-{a}}\sqcap Trans(D)
			...
			for(OWLNamedIndividual a:this.namedElements){
				...
				if(!tmpOneOf2.isEmpty()){
					disjuncts.add(this.factory.getOWLObjectIntersectionOf(
							this.factory.getOWLObjectOneOf(tmpOneOf2),newFiller));
				}
			}
	...
	public OWLClassExpression visit(OWLObjectAllValuesFrom ce) {	
		...
		if(ce.getProperty() instanceof OWLObjectEpistemicRole){
			...
			OWLClassExpression notD = this.factory.getOWLObjectComplementOf(ce.getFiller()).accept(this);
			Set<OWLIndividual> instSet = getInstances(notD);
			if(instSet.isEmpty()){
				return this.factory.getOWLNothing();
			}
			OWLClassExpression newD = this.factory.getOWLObjectOneOf(instSet);
			Set<OWLIndividual> set = getInstances(this.factory.getOWLObjectSomeValuesFrom(prop, newD));
			if(set.isEmpty()){
				return this.factory.getOWLNothing();
			}
			else{
	...
	//	************************************** Qualified Number Restriction  \geq nKR.D **********************************************************
	@Override
	public OWLClassExpression visit(OWLObjectMinCardinality ce) {
		...
		if(ce.getProperty() instanceof OWLObjectEpistemicRole){
			...
			OWLClassExpression newD = this.factory.getOWLObjectOneOf(instSet);
			OWLClassExpression newCE = this.factory.getOWLObjectMinCardinality(ce.getCardinality(), prop, newD);
			...
			return this.factory.getOWLObjectMinCardinality(ce.getCardinality(), ce.getProperty(), newFiller);
		}
	}
	...
	public OWLClassExpression visit(OWLObjectMaxCardinality ce) {
		...
		if(ce.getProperty() instanceof OWLObjectEpistemicRole){
			...
			if(instSet.isEmpty()){ return this.factory.getOWLNothing();}
			OWLClassExpression newD = this.factory.getOWLObjectOneOf(instSet);
			OWLClassExpression newCE = this.factory.getOWLObjectMinCardinality(ce.getCardinality()+1, prop, newD);
			Set<OWLIndividual> set = getInstances(newCE);
			if(set.isEmpty()){ 
				return this.factory.getOWLNothing(); 
			}
			...
			return this.factory.getOWLObjectMaxCardinality(ce.getCardinality(), ce.getProperty(), newFiller);
		}
	}
	...
	private Set<OWLIndividual> getInstances (OWLClassExpression ce){
		...
		this.kCount++;
		Set<OWLIndividual> set = new HashSet<OWLIndividual>();
		Iterator<Node<OWLNamedIndividual>> it = this.reasoner.getInstances(ce, false).iterator();
		while(it.hasNext()){
