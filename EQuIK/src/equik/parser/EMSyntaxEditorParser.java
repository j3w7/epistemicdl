...
import uk.ac.manchester.cs.owl.owlapi.OWLObjectEpistemicConceptImpl;
import uk.ac.manchester.cs.owl.owlapi.OWLObjectEpistemicRoleImpl;
		...
		while (true) {
			...
			if (sect.equalsIgnoreCase(SUB_CLASS_OF)) {
				...
				if (!isAllowableEmptyClassFrameSection()) {
					...
					for (OWLOntology ont : onts) {
						axioms.add(new OntologyAxiomPair(ont, dataFactory
								.getOWLHasKeyAxiom(cls, props, annos)));
					}
				}
